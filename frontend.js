...
import { createThread, addUserMessageToThread, runModel } from 'backend/chatgpt.jsw';
...
$w.onReady(async function () {
    ...
    $w('#buttonPrompt1').onClick(() => {
        const userMessage = $w('#textPrompt1').text
        $w("#shortcut").collapse();
        $w("#htmlComponent").show();
        processMessage(userMessage);
    })

    $w('#buttonPrompt2').onClick(() => {
        const userMessage = $w('#textPrompt2').text
        $w("#shortcut").collapse();
        $w("#htmlComponent").show();
        processMessage(userMessage);
    })

    $w('#buttonPrompt3').onClick(() => {
        const userMessage = $w('#textPrompt3').text
        $w("#shortcut").collapse();
        $w("#htmlComponent").show();
        processMessage(userMessage);
    })
...
async function processMessage(userMessage) {
    ...
    if (!threadId && (isPlanActive || messageCount < 3)) {
        console.log("making thread")
        const thread = await createThread()
        threadId = thread.id;
    }
    if (isPlanActive) {
        await sendMessageToGPT(userMessage)
...
async function sendMessageToGPT(userMessage) {
    ...
    await addUserMessageToThread(threadId, userMessage)
    const result = await runModel(threadId);
    if (result.success) {
        console.log("Response from assistant:", result.response);

        addMessageToChat("gpt", result.response);
    } else {
